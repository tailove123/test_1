...
void send_message(char* , char *, int *, char *);
int user_exists(char* , char *);
void mk_txt(FILE *, char *, char *, struct tm *);
void text_list_func(char *, char *,int *, int *);
...
int main() 
{
    ...
    char folder[SIZE] = "/ID";
    char receiver_mail_dir_path[SIZE];
        ...
        while(user_input == 1)
        {
            ...
            if (strcmp(login_u_ID, "admin") == 0) // 관리자 계정으로 로그인한 경우
            {
                ...
                else if (c_input == 'f')
                {
                    ...
                    {
                        printf("%3d.   %s\t %s\t %d\t %s-%s-%s\n", i, users_info[i].u_ID, users_info[i].u_name, users_info[i].u_age, users_info[i].u_phone_1,users_info[i].u_phone_2, users_info[i].u_phone_3);
                    }
            ...
            else // 기존유저
            {
                ...
                if (c_input == 'w') //w.txt 만들기
                {
                    
                    mk_txt(fp, login_u_ID, cwd_path, ptr_tm);
                    continue;
                }
                else if (c_input == 'a') //a.txt 목록
                {               
                    text_list_func(login_u_ID, cwd_path, &send_switch, &user_mail_select);
                    continue;          
                }
                else if (c_input == 't') //t.txt 전송하기
                {
                    printf("1.작성 후 전송   2.목록 불러오기\n");
                    scanf("%d",&i_input);
                    if (i_input == 1)
                    {
                        send_message(login_u_ID, cwd_path, &send_switch, receiver_mail_dir_path);
                    }
                    else if (i_input == 2)
                    {
                        send_switch = 1;
                ...
                else if (c_input == 'z') //z. 종료 / 로그아웃 ==> 이따 봐봄
                {
                    printf("1.로그아웃  2.종료하기");
                    scanf("%d",&i_input);
                    if (i_input == 1)
...
void join_func(int count, new_user new_user_info, users *users_info, char *folder,  char *csv_path, char *cwd_path)
{
    ...
    fprintf(fp, "%d,%s,%s,%s,%d,%s,%s,%s\n", count, new_user_info.new_id, new_user_info.new_pw, new_user_info.new_name, new_user_info.new_age, new_user_info.new_phone_1, new_user_info.new_phone_2, new_user_info.new_phone_3);
    fclose(fp);
            ...
            case 2: // 이름으로 찾기
            {
                ...
                else
                {
                    ...
                    for (int i = 0 ; i < count ; i++)
                    {
                        ...
                        if (strcmp(find_id, users_info[i].u_name) == 0) // ID가 있는 경우
                        {
                            user_num++;
                        }
                    }
...
// .TXT 전송하기
void send_message(char* login_u_ID,  char *cwd_path, int *send_switch, char *receiver_mail_dir_path) //메세지 보내기
{
...
// 텍스트 목록
void text_list_func(char *login_u_ID,  char *cwd_path, int *send_switch, int *user_mail_select)
{
    int start_page = 1; // 시작 페이지
    int end_page; // 끝 페이지
    int start_num = 0; // 각 페이지 시작 번호
    ...
    while (1)
    {
        ...
        if (mail_count >= 90)
        {
            printf("⚠️메일 갯수가 90개 이상입니다! 100개 초과시 옛날 메일은 삭제됩니다!⚠️\n");
        }
        ...
        else if (str_num[0] != 0) // 숫자를 입력했을 때
        {
            if (strlen(str_str) == 0) // 숫자만 입력했을 때
            {
                if(*send_switch ==1)
                {
                    *user_mail_select = atoi(user_page_select);
                    break;
                }
                printf("[r. 읽기 | d. 삭제하기]]\n");
                scanf("%s", user_page_select);
