        ...
        while(user_input == 1)
        {
            ...
            else // 기존유저
            {
                ...
                else if (c_input == 't') //t.txt 전송하기
                {
                    ...
                    {
                        send_text_list_func(login_u_ID,users_info ,cwd_path);
                    }
                    else
                    {
...
// 목록에서 메일보내기
void send_text_list_func(char *login_u_ID,  users *users_info ,char *cwd_path)
{
    int start_page = 1; // 시작 페이지
    int end_page; // 끝 페이지
    int start_num = 0; // 각 페이지 시작 번호
    ...
    while (1)
    {
        ...
        else if (str_num[0] != 0) // 숫자를 입력했을 때
        {
            ...
            if (strcmp(str_str, "s") == 0) // 읽기를 선택했을 때
            {
                system("clear");
                strcpy(user_dir, "/home/boeun/Desktop/3H_mail_copy2/ID/");
                strcpy(user_dir,cwd_path);
