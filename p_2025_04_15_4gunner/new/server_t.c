...
void *client_handler(void *arg) {
    ...
    if (result == 1 && strcmp(cmd, "login") == 0) {
        while (1) {
            ...
            {
                char key[50], val[100];
                Book p_found[MAX_BOOKS];
                // Book *p_found = malloc(sizeof(Book));
                // memset(p_found, 0, sizeof(Book));
                ...
                {
                    printf("%d, %d, %s, %s\n", count, i, p_found[i].title, p_found[i].author);
                }
