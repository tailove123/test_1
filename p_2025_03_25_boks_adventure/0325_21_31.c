...
int main(){
    ...
    int live_hp = max_hp;                                                     // 최대 체력에서 총물리마법데미지를 뺀값         
    ...
    int monster_kill_exp[ARR_SIZE] =                {100,          1000,          2000,         3000,     4000,      5000, 0, 0};
    //경험치보상
        ...
        while(floor != 0)                   ///////////// 던전 메뉴 /////////////
        {
            if (map_input =='1')
            {
                ...
                else if (village_select ==2)  //인벤토리 열기
                {
                    ...
                    int inven_switch = 1;
                    while (inven_switch && tp_switch==0)
                    {
                        int input;
                        ...
                        while (input == 1)
                        {
                            ...
                            printf("1. 장비교체  2. 소비아아이템사용  [나가려면 0] : ");
                            int user_select_janbi_orcons;
                            scanf("%d", &user_select_janbi_orcons);

                            if(user_select_janbi_orcons==1)
                            {
            ...
            while(floor != 0)     
            {
                ...
                switch (user_war_select)
                {
                    case 1:                 //물리공격 선택
                    {
                        ...
                        AD = live_atk + ((1 + MY_EQUIP[0][2]) * (1 + (MY_EQUIP[0][4] * 0.01)) * (1 + (MY_EQUIP[0][5] * 0.01))) * (ALLTK_PER*0.01) * cri_success;

                        printf("%d,%d,%d,%d,%d,%d",live_atk,MY_EQUIP[0][2],MY_EQUIP[0][4] * 0.01,MY_EQUIP[0][5] * 0.01,ALLTK_PER*0.01,cri_success);
                    ...
                    case 2:                  //마법공격 선택
                    {
                        ...
                        {
                            AP = ((live_mtk * ((1 + skill_magic_m[user_skill_select-1]) + 5) * ((1 + ALLTK_PER) * 0.01))* (1 + weakness_point)) ;
                        }
                        else                               //마법배율 장비 특수옵션 없을 때 계산식
                        {
                            AP = ((( live_mtk * (1 + skill_magic_m[user_skill_select-1])) * ((1 + ALLTK_PER) * 0.01))* (1 + weakness_point)) ;
                        }
                        printf("%d\n",AP);
                ...
                if (monster_max_hp <= 0)
                {
                    ...
                    else if (mon_name_num==7)
                    {
                        return 0;
                    }
                    else
                    {
                        if (plag_named==1)                    //네임드면 골드 2배
                        {
                            drop_gold = (monster_fix_gold + (rand() % monster_random_gold[mon_name_num]));//100
                            gold = gold + (drop_gold*2);   
                        }
                        ...
                        live_exp += monster_kill_exp[mon_name_num];
                        
                        if (max_exp<=live_exp)             //경험치량 최대에 도달하면 레벨업
                        {
                            live_lv+=1;
                            live_exp = live_exp - max_exp;
...
{   
    ...
    else if ((floor ==2)&&(plag_named==1))          //2층에 있으면
    {
        mon_name_num = draw_2;
        if (mon_name_num ==0)
        {
        ...
        {
            monster_max_hp = (monster_fix_hp[draw_2] + (rand() % monster_random_hp[draw_2]))*2;    //몬스터 체력결정
            monster_max_ac = (monster_fix_ac[draw_2] + (rand() % monster_random_ac[draw_2]))*2;   //몬스터 방어력 결정
            monster_max_atk = (monster_fix_atk[draw_2] + (rand() % monster_random_atk[draw_2]))*2;    //몬스터 공격력 결정
        }
    }
    else if (floor ==3&&(plag_named==1))          //3층에 있으면
    {
        mon_name_num = draw_2+1;
        monster_max_hp = (monster_fix_hp[draw_2+1] + (rand() % monster_random_hp[draw_2+1]))*2;    //몬스터 체력결정
        monster_max_ac = (monster_fix_ac[draw_2+1] + (rand() % monster_random_ac[draw_2+1]))*2;   //몬스터 방어력 결정
        monster_max_atk = (monster_fix_atk[draw_2+1] + (rand() % monster_random_atk[draw_2+1]))*2;    //몬스터공격력 결정
    }
    else if (floor ==4&&(plag_named==1))              //4층에 있으면
    {
        mon_name_num = draw_2+2;
        monster_max_hp = (monster_fix_hp[draw_2+2] + (rand() % monster_random_hp[draw_2+2]))*2;    //몬스터 체력결정
        monster_max_ac = (monster_fix_ac[draw_2+2] + (rand() % monster_random_ac[draw_2+2]))*2;   //몬스터 방어력 결정
        monster_max_atk = (monster_fix_atk[draw_2+2] + (rand() % monster_random_atk[draw_2+2]))*2;    //몬스터 공격력 결정
    }
    else if (floor ==5&&(plag_named==1))             //5층에 있으면
    {
        mon_name_num = draw_3+3;
        monster_max_hp = (monster_fix_hp[draw_3+3] + (rand() % monster_random_hp[draw_3+3]))*2;    //몬스터 체력결정
        monster_max_ac = (monster_fix_ac[draw_3+3] + (rand() % monster_random_ac[draw_3+3]))*2;   //몬스터 방어력 결정
        monster_max_atk = (monster_fix_atk[draw_3+3] + (rand() % monster_random_atk[draw_3+3]))*2;    //몬스터 공격력 결정
    }
    ...
    else if (floor ==5)             //5층에 있으면
    {
        ...
        else if (map[user_y-1][user_x]==3 || map[user_y+1][user_x]==3 || map[user_y][user_x-1]==3 || map[user_y][user_x+1]==3)       //캐릭터 주위에 이동녀크가 있을 시
        {
            mon_name_num = 7;
            monster_max_hp = monster_fix_hp[7] + (max_hp*monster_random_hp[7]);    //몬스터 체력결정
            monster_max_ac = monster_fix_ac[7] + (monster_random_ac[7]);   //몬스터 방어력 결정
