...
float skill_monster_pro_cal(int monster_pro_weak,int user_skill_properties)
{
    ...
    if (monster_pro_weak == 1 && user_skill_properties == 3)
    {
       weakness_point == 0.5; 
       return weakness_point;
    }
    else if (monster_pro_weak == 3 && user_skill_properties == 1)
    {
        weakness_point == 2;
        return weakness_point; 
    }
    else if (monster_pro_weak  == 0 || user_skill_properties == 0)
    {
        weakness_point == 1;
        return weakness_point;
    }
            ...
            while(floor != 0)     
            {
                ...
                switch (user_war_select)
                {
                    case 1:                 //물리공격 선택
                    {
                        ...
                        AD = ((live_atk + ((MY_EQUIP[0][2] * (MY_EQUIP[0][4] * 0.01))* (MY_EQUIP[0][5] * 0.01))) * ALLTK_PER) * cri_success;
                        // printf("%d",AD);
