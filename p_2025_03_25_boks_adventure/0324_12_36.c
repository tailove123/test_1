...
float skill_monster_pro_cal(int monster_pro_weak,int user_skill_properties){
    float weakness_point;

    if (monster_pro_weak == 0 && user_skill_properties == 2)
    {
       weakness_point == 0.5; 
       return weakness_point;
    }
    else if (monster_pro_weak == 2 && user_skill_properties == 0)
    {
        weakness_point == 2;
        return weakness_point; 
    }
    else if (monster_pro_weak > user_skill_properties)
    {
        weakness_point == 0.5; 
        return weakness_point;
    } 
...
int main(){
    ...
    int AD; //총물리데미지
    int AP; //총마법데미지
    int ALLTK_PER = 1; // 일단 장비에서 특수옵션미구현이라 1로

    // 골드
    int START_GOLD = 10000;
    int drop_gold;
    ...
    int return_stp;                                             // 스텟 리턴값 (초기화시 스텟포인트 리턴 )
    ...
    int live_mtk = START_INT_ + added_mtk;
    ...
    float added_cri = START_CRI + (DEX_UP_CRI * added_dex);
    ...
    while (1)
    {
        ...
        while(floor != 0)                   ///////////// 던전 메뉴 /////////////
        {
            if (map_input =='1')
            {
                ...
                else if (village_select == 2) // 메뉴 나가기
                {
                    map_not_change();
                    break;
                }           
            ...
            while(floor != 0 && turn_switch == 0)     
            {
                ...
                switch (user_war_select)
                {
                    ...
                    case 2:
                    {
                        ...
                        AP = ((( live_mtk * skill_magic_m[user_skill_select-1] ) * ALLTK_PER) * weakness_point) *  live_cri;
                        printf("%d\n",AP);
...
void map_print()
{
    ...
    while (plag_mapout == 0)   //전투든 메뉴든 와일문 나가는 플래그
    {
        ...
        while(map_input != '1')
        {
            ...
            // 플레이어 좌표와 포탈 좌표가 같다면 그리고 몬스터가 없다면 층수 ++ 컨티뉴 후 반복문 나가기
            if ((user_y==portal_y) && (user_x==portal_x) && (mon_num==0))
            {
                floor += 1;
                map_change();
                break;
...
{   
    ...
    {
        mon_name_num = draw_2;
        monster_max_hp = monster_fix_hp[draw_2] + (rand() % monster_random_hp[draw_2]);    //오크전사 체력결정
        monster_max_ac = monster_fix_ac[draw_2] + (rand() % monster_random_ac[draw_2]);   //오크전사 방어력 결정
        monster_max_atk = monster_fix_atk[draw_2] + (rand() % monster_random_atk[draw_2]);    //오크전사 공격력 결정
    }
    else if (floor ==3)          //3층에 있으면
    {
        mon_name_num = draw_2+1;
        monster_max_hp = monster_fix_hp[draw_2+1] + (rand() % monster_random_hp[draw_2+1]);    //오크전사 체력결정
        monster_max_ac = monster_fix_ac[draw_2+1] + (rand() % monster_random_ac[draw_2+1]);   //오크전사 방어력 결정
        monster_max_atk = monster_fix_atk[draw_2+1] + (rand() % monster_random_atk[draw_2+1]);    //오크전사 공격력 결정
    }
    else if (floor ==4)              //4층에 있으면
    {
        mon_name_num = draw_2+2;
        monster_max_hp = monster_fix_hp[draw_2+2] + (rand() % monster_random_hp[draw_2+2]);    //오크전사 체력결정
        monster_max_ac = monster_fix_ac[draw_2+2] + (rand() % monster_random_ac[draw_2+2]);   //오크전사 방어력 결정
        monster_max_atk = monster_fix_atk[draw_2+2] + (rand() % monster_random_atk[draw_2+2]);    //오크전사 공격력 결정
    }
    else if (floor ==5)             //5층에 있으면
    {
        ...
        // {
        //     보스 능력치 업데이트 되었으면 넣어야 함.
        // }
        // else                    //나머지 일반몹
        // {
            mon_name_num = draw_3+3;
            monster_max_hp = monster_fix_hp[draw_3+3] + (rand() % monster_random_hp[draw_3+3]);    //오크전사 체력결정
            monster_max_ac = monster_fix_ac[draw_3+3] + (rand() % monster_random_ac[draw_3+3]);   //오크전사 방어력 결정
            monster_max_atk = monster_fix_atk[draw_3+3] + (rand() % monster_random_atk[draw_3+3]);    //오크전사 공격력 결정
        // }
